...
import com.example.grus95.baselibrary.recycler_refresh_layout.realization_demo.adapter.RecyclerListAdapter;
...
/**
 * Created by grus95 on 2016/12/6.
 */

public class TVShowRecyclerFragment extends RecyclerFragment<TVShowModel> {
    ...
    private final List<TVShowModel> mItemList = new ArrayList<>();

    private int mSimulateStatus;

    public static TVShowRecyclerFragment newInstance() {
        return new TVShowRecyclerFragment();
    }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (itemId == R.id.refresh_first) {
            mSimulateStatus = SIMULATE_FRESH_FIRST;
        } else if (itemId == R.id.refresh_no_data) {
            mSimulateStatus = SIMULATE_FRESH_NO_DATA;
        } else if (itemId == R.id.refresh_failure) {
...
//        getRecyclerRefreshLayout().setRefreshView(new SimpleRefreshView(getActivity()), layoutParams);
    ...
    public RecyclerListAdapter createAdapter() {
        return new RecyclerListAdapter() {
            {
                addViewType(TVShowModel.class, new ViewHolderFactory<ViewHolder>() {
                    @Override
                    ...
                    mHandler.post(new Runnable() {
                        ...
                        public void run() {
                            if (mSimulateStatus == SIMULATE_FRESH_FAILURE) {
                                ...
                                listener.onSuccess(TVShowFactory.createOpenProjects());
                            }
    ...
    private interface RequestListener {
        void onSuccess(List<TVShowModel> openProjectModels);
    ...
    private class ItemInteractionListener extends InteractionListener {
        ...
        public void requestRefresh() {
            simulateNetworkRequest(new RequestListener() {
                @Override
                public void onSuccess(List<TVShowModel> openProjectModels) {
                    mItemList.clear();
    ...
    private class ItemViewHolder extends RecyclerListAdapter.ViewHolder<TVShowModel> {
        ...
        public ItemViewHolder(@NonNull ViewGroup parent) {
            super(LayoutInflater.from(getActivity()).inflate(R.layout.layout_tvshow_item, parent, false));

            mTvTitle = (TextView) itemView.findViewById(R.id.tvshow_title);
            mTvContent = (TextView) itemView.findViewById(R.id.tvshow_content);
            mTvAuthor = (TextView) itemView.findViewById(R.id.tvshow_author);

            mLlContentPanel = (LinearLayout) itemView.findViewById(R.id.tvshow_content_panel);
        }

        @Override
        public void bind(final TVShowModel item, int position) {
            mTvTitle.setText(item.getTitle());
            mTvContent.setText(item.getContent());
            mTvAuthor.setText(item.getAuthor());

            mLlContentPanel.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View view) {
                    Intent intent = new Intent(getContext(), TVShowActivity.class);
                    intent.putExtra("TVTitle", item.getTitle());
                    intent.putExtra("TVUrl", item.getContent());
